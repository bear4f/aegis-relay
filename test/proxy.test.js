...
test('toolbox-style base paths are not duplicated after login redirects',async()=>{
  ...
  try{
    ...
    const key=deriveKey('g'.repeat(32)),route={id:'toolbox',alias:'magpie',enabled:true,accessMode:'alias_only',upstreams:[`http://127.0.0.1:${gp}/tool-key/http/origin.example:8096`],allowPrivate:true,directStream:false},store={data:{routes:[route]}};
