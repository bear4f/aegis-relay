...
async function loadAccess(){const host=$('#access-list');host.innerHTML='<article class="panel empty-state"><h3>正在汇总访问记录…</h3></article>';try{const d=await call('/access');const nodes=(d.nodes||[]).filter(n=>state.routes.some(r=>r.id===n.id));if(!nodes.length){host.innerHTML='<article class="panel empty-state"><h3>暂无访问记录</h3><p>有人通过反代观看后，这里会显示其 IP 与设备。</p></article>';return}
  const fmt=t=>t?new Date(Number(t)).toLocaleString():'—';
  host.innerHTML=nodes.map(n=>{const rows=(n.viewers||[]).map(v=>`<tr><td><code>${escapeHtml(v.ip)}</code></td><td>${escapeHtml(v.deviceName||v.client||'—')}${v.client&&v.deviceName?`<br><small class="muted">${escapeHtml(v.client)}</small>`:''}</td><td><small class="muted">${escapeHtml((v.ua||'').slice(0,60))}</small></td><td>${escapeHtml(v.machine||'—')}</td><td>${v.hits||0}</td><td>${fmt(v.lastSeen)}</td></tr>`).join('');
    return `<article class="panel access-card ${n.suspicious?'suspicious':''}"><div class="access-head"><div><h3>${escapeHtml(n.name)} <small class="muted">/${escapeHtml(n.alias)}/</small></h3><div class="access-stats"><span class="pill ${n.suspicious?'warn':'info'}">不同设备 ${n.distinctDevices}</span><span class="pill off" title="大陆动态 IP 会经常变，仅供参考，不用于预警">不同 IP ${n.distinctIps}</span>${n.threshold?`<span class="pill ${n.suspicious?'warn':'off'}">设备阈值 ${n.threshold}</span>`:'<span class="pill off">未设阈值</span>'}${n.suspicious?'<span class="pill danger">⚠ 疑似被分发</span>':''}</div></div></div>${rows?`<div class="table-wrap"><table><thead><tr><th>IP</th><th>设备 / 客户端</th><th>User-Agent</th><th>机器</th><th>请求数</th><th>最近访问</th></tr></thead><tbody>${rows}</tbody></table></div>`:'<p class="muted">该节点暂无观看记录。</p>'}</article>`}).join('')}catch(e){host.innerHTML=`<article class="panel empty-state"><h3>加载失败</h3><p>${escapeHtml(e.message)}</p></article>`}}
