...
$('#run-diagnostic').onclick=async()=>{const id=value('#diag-node');if(!id)return toast('请先创建节点',true);$('#diagnostic-result').innerHTML='<article class="panel empty-state"><h3>正在探测回源…</h3></article>';try{const d=await call(`/diagnostics/${id}`);const section=(title,list)=>`<article class="panel diag-card"><span class="eyebrow">${title}</span><h3>${escapeHtml(d.node.name)}</h3>${(list||[]).map(x=>{const lat=Number(x.latencyMs),lcls=!x.online?'bad':lat<300?'good':lat<800?'warn':'bad';return`<div class="diag-line"><span>连接状态</span><b class="${x.online?'good':'bad'}">${x.online?'在线':'失败'}</b></div><div class="diag-line"><span>HTTP 状态</span><b>${x.status||'—'}</b></div><div class="diag-line"><span>响应延迟</span><b class="${lcls}">${isFinite(lat)?lat+' ms':x.latencyMs}</b></div><div class="diag-line"><span>Emby 版本</span><b>${escapeHtml(x.embyVersion||'—')}</b></div>${x.tls?`<div class="diag-line"><span>TLS</span><b class="${x.tls.authorized?'good':'warn'}">${x.tls.authorized?'有效':'未验证'}</b></div><div class="diag-line"><span>证书到期</span><b>${escapeHtml(x.tls.validTo||'—')}</b></div>`:''}${x.error?`<div class="diag-line"><span>错误</span><b class="bad">${escapeHtml(x.error)}</b></div>`:''}`}).join('')}</article>`;$('#diagnostic-result').innerHTML=section('主回源',d.main)+(d.playback?section('播放回源',d.playback):section('播放回源',[{online:true,status:'跟随主回源',latencyMs:'—'}]))+`<article class="panel diag-card"><span class="eyebrow">HEADER PREVIEW</span><h3>请求头策略</h3>${Object.entries(d.headerPreview).map(([k,v])=>`<div class="diag-line"><span>${escapeHtml(k)}</span><b>${escapeHtml(v)}</b></div>`).join('')}</article>`;await loadRuntime()}catch(e){toast(e.message,true)}};
...
async function loadAgents(){
  ...
  $('#agent-grid').innerHTML=state.agents.map(a=>{const change=a.domainChange,working=change&&['pending','applying'].includes(change.state),failed=change?.state==='failed',desired=working||failed?change.desiredDomain:a.domain,status=change?`<div class="node-test show"><span class="${failed?'bad':working?'warn':'good'}">●</span>&nbsp;${escapeHtml(change.message||'')}${change.desiredDomain&&change.desiredDomain!==a.domain?` · 目标 ${escapeHtml(change.desiredDomain)}`:''}</div>`:'';return`<article class="node-card agent-card" data-id="${escapeHtml(a.id)}" data-status="${escapeHtml(a.status)}"><div class="node-card-head"><div><h3>${escapeHtml(a.name)}</h3><span class="node-path">${escapeHtml(a.transport==='loopback'?'本机环回':'安全轮询')}</span></div><span class="pill ${a.inSync?'on':'warn'}">${a.inSync?'配置已同步':'等待同步'}</span>${a.agentVersion&&state.panelVersion&&a.agentVersion!==state.panelVersion?`<span class="pill warn" title="该机器仍在运行旧版本代理程序">待升级 ${escapeHtml(a.agentVersion)}</span>`:''}</div><div class="tag-row"><span class="pill ${a.status==='online'?'on':'warn'}">${labels[a.status]||'失联'}</span><span class="pill ${working?'warn':a.applyState==='active'?'info':'warn'}">${working?'正在配置域名':a.applyState==='active'?'代理运行中':'已注册 · 等待配置'}</span></div><div class="node-meta"><div><small>当前生效域名</small><strong>${escapeHtml(a.domain||'尚未设置')}</strong></div><div><small>已选节点</small><strong>${a.routeIds.length} 个</strong></div><div><small>配置版本</small><strong>revision ${a.appliedRevision} / ${a.desiredRevision}</strong></div><div><small>最近回执</small><strong>${a.lastSeen?new Date(a.lastSeen).toLocaleString():'尚未连接'}</strong></div></div><div class="agent-editor"><label>代理域名<input class="agent-domain" value="${escapeHtml(desired||'')}" placeholder="hk.example.com" ${working?'disabled':''}><small>${a.automaticDomainSwitch?'保存后会自动配置 Nginx、申请证书并停用旧代理域名。':'远程机器的 HTTPS 域名。'}</small></label><details class="agent-routes"><summary>部署节点<span class="routes-count">已选 ${a.routeIds.length} / ${state.routes.length}</span></summary>${state.routes.length>8?'<div class="route-filter"><input class="agent-route-search" placeholder="搜索节点"></div>':''}<div class="agent-route-list">${state.routes.map(r=>`<label data-search="${escapeHtml((r.name+' '+r.alias).toLowerCase())}"><input type="checkbox" data-route-id="${r.id}" ${a.routeIds.includes(r.id)?'checked':''}><span>${escapeHtml(r.name)} <small>/${escapeHtml(r.alias)}/</small></span></label>`).join('')||'<small>请先创建 Emby 节点</small>'}</div></details><div class="node-actions"><button class="btn primary save-agent" ${working?'disabled':''}>${a.automaticDomainSwitch?'保存并切换域名':'保存机器配置'}</button>${a.canDelete?'<button class="btn ghost danger delete-agent">从面板删除</button>':''}</div></div>${status||a.error?status||`<div class="node-test show"><span class="bad">●</span>&nbsp;${escapeHtml(a.error)}</div>`:''}${a.agentVersion&&state.panelVersion&&a.agentVersion!==state.panelVersion?`<div class="node-test show"><span class="warn">●</span>&nbsp;代理程序 ${escapeHtml(a.agentVersion)} 落后于面板 ${escapeHtml(state.panelVersion)}，在该机器执行：<code>sudo aegis-relay-agent update</code></div>`:''}</article>`}).join('')||'<article class="panel empty-state"><svg viewBox="0 0 24 24"><circle cx="12" cy="5" r="2"/><circle cx="5" cy="18" r="2"/><circle cx="19" cy="18" r="2"/><path d="M12 7v4M6.7 16.8l3.8-4M17.3 16.8l-3.8-4"/></svg><h3>添加第一台代理机器</h3><p>生成一次性安装指令，机器注册后会自动出现。</p><button class="btn primary open-agent">添加机器</button></article>';
  $('#agent-grid').querySelectorAll('.agent-card').forEach(card=>{const agent=state.agents.find(item=>item.id===card.dataset.id);const search=card.querySelector('.agent-route-search');if(search)search.oninput=()=>{const q=search.value.trim().toLowerCase();card.querySelectorAll('.agent-route-list label').forEach(l=>{l.style.display=!q||(l.dataset.search||'').includes(q)?'':'none'})};const counter=card.querySelector('.routes-count');if(counter)card.querySelectorAll('[data-route-id]').forEach(box=>box.addEventListener('change',()=>{counter.textContent=`已选 ${card.querySelectorAll('[data-route-id]:checked').length} / ${state.routes.length}`}));card.querySelector('.save-agent').onclick=async()=>{try{const routeIds=[...card.querySelectorAll('[data-route-id]:checked')].map(input=>input.dataset.routeId),domain=card.querySelector('.agent-domain').value;await call(`/agents/${agent.id}`,{method:'PATCH',body:JSON.stringify({domain,routeIds})});toast(agent.automaticDomainSwitch&&domain!==agent.domain?'域名切换已提交，正在申请证书':'代理机器配置已保存');await refreshAll();await loadAgents();if(agent.automaticDomainSwitch&&domain!==agent.domain)watchDomainSwitch()}catch(e){toast(e.message,true)}};const remove=card.querySelector('.delete-agent');if(remove)remove.onclick=async()=>{if(!confirm(`确认从面板删除“${agent.name}”？请先在对应机器执行卸载。`))return;try{await call(`/agents/${agent.id}`,{method:'DELETE'});toast('代理机器已从面板删除');await loadAgents()}catch(e){toast(e.message,true)}}});
  $$('.open-agent').forEach(button=>button.onclick=openAgentModal);
}
let domainWatchTimer=null;
function watchDomainSwitch(){clearTimeout(domainWatchTimer);domainWatchTimer=setTimeout(async()=>{try{await loadAgents();const local=state.agents.find(a=>a.automaticDomainSwitch),working=local?.domainChange&&['pending','applying'].includes(local.domainChange.state);if(working)watchDomainSwitch();else if(local?.domainChange?.state==='active'){await refreshAll();toast(`代理域名 ${local.domain} 已生效，客户端地址已更新`)}else if(local?.domainChange?.state==='failed')toast(local.domainChange.message||'域名切换失败',true)}catch{watchDomainSwitch()}},4000)}
...
async function openAgentModal(){try{const deployment=await call('/deployment');if(!deployment.certificateEmail){toast('请先在部署向导保存统一证书邮箱',true);await switchPage('deployment');return}setValue('#agent-name','');setValue('#agent-domain-new','');$('#agent-command-wrap').classList.add('hidden');$('#agent-enroll-form').classList.remove('hidden');$('#agent-enroll-routes').innerHTML=state.routes.map(route=>`<label><input type="checkbox" data-enroll-route="${route.id}"><span>${escapeHtml(route.name)} <small>/${escapeHtml(route.alias)}/</small></span></label>`).join('')||'<small>暂时不部署节点，注册后仍可选择。</small>';$('#agent-modal').classList.remove('hidden');setTimeout(()=>$('#agent-name').focus(),0)}catch(e){toast(e.message,true)}}
