...
const titles={dashboard:'仪表盘',nodes:'节点管理',agents:'代理机器',traffic:'流量统计',access:'访问监控',diagnostics:'故障诊断',audit:'安全审计',notifications:'通知提醒',account:'个人设置',deployment:'部署向导'};
...
function statusPills(r){const pills=[`<span class="pill ${r.enabled?'on':'off'}">${r.enabled?'运行中':'已停用'}</span>`,`<span class="pill ${r.accessMode==='key'?'lock':'off'}">${r.accessMode==='key'?'密钥':'公开'}</span>`];if(r.clientProfile&&r.clientProfile.enabled)pills.push(`<span class="pill disguise">伪装${r.clientProfile.client?'：'+escapeHtml(r.clientProfile.client):''}</span>`);if(r.streamRewrite&&r.streamRewrite.enabled)pills.push(`<span class="pill split">分离推流</span>`);const dn=state.dashboard&&state.dashboard.nodes&&state.dashboard.nodes.find(n=>n.id===r.id);if(dn&&dn.suspicious)pills.push(`<span class="pill danger" title="30 天内不同设备 ${dn.distinctDevices} 超过阈值（动态 IP 不计入）">⚠ 疑似被分发</span>`);return pills.concat(r.tags.map(t=>`<span class="tag">${escapeHtml(t)}</span>`)).join('')}
...
$('#refresh-access').onclick=loadAccess;
$('#add-channel').onclick=async()=>{if(!state.editing)return toast('请先保存节点，再添加分发通道',true);const route=state.routes.find(r=>r.id===state.editing);if(!route)return;try{await call(`/routes/${route.id}/channels`,{method:'POST',body:JSON.stringify({label:value('#channel-label').trim()})});setValue('#channel-label','');await renderChannels(route);toast('分发通道已添加')}catch(e){toast(e.message,true)}};
...
// Renumber with gaps so a later single move only has to rewrite the machines that actually moved.
async function persistAgentOrder(){
  const updates=state.agents.map((agent,i)=>({agent,order:(i+1)*10})).filter(x=>Number(x.agent.sortOrder)!==x.order);
  if(!updates.length)return;
  try{for(const {agent,order} of updates){await call(`/agents/${agent.id}`,{method:'PATCH',body:JSON.stringify({sortOrder:order})});agent.sortOrder=order}toast('排序已保存')}
  catch(error){toast(error.message,true);await loadAgents()}
}
...
// Card drag mirrors the node table's row drag; cards sit in a grid, so left/right halves decide the
// drop side instead of top/bottom.
function bindAgentDrag(card,agent){
  const handle=card.querySelector('.drag-handle');
  if(!handle||!handle.getAttribute('draggable'))return;
  const side=event=>{const box=card.getBoundingClientRect();return event.clientX>box.left+box.width/2};
  handle.addEventListener('dragstart',event=>{dragAgentId=agent.id;card.classList.add('dragging');event.dataTransfer.effectAllowed='move';try{event.dataTransfer.setData('text/plain',agent.id)}catch{}});
  handle.addEventListener('dragend',()=>{dragAgentId=null;card.classList.remove('dragging');$$('.agent-card').forEach(el=>el.classList.remove('drop-above','drop-below'))});
  card.addEventListener('dragover',event=>{
    if(!dragAgentId||dragAgentId===agent.id)return;
    event.preventDefault();event.dataTransfer.dropEffect='move';
    const after=side(event);card.classList.toggle('drop-below',after);card.classList.toggle('drop-above',!after);
  });
  card.addEventListener('dragleave',()=>card.classList.remove('drop-above','drop-below'));
  card.addEventListener('drop',event=>{
    if(!dragAgentId||dragAgentId===agent.id)return;
    event.preventDefault();
    const after=side(event),from=state.agents.findIndex(item=>item.id===dragAgentId);
    if(from<0)return;
    const [moved]=state.agents.splice(from,1);
    let to=state.agents.findIndex(item=>item.id===agent.id);
    if(to<0)to=state.agents.length-1;
    state.agents.splice(after?to+1:to,0,moved);
    dragAgentId=null;renderAgents();persistAgentOrder();
  });
}
function renderAgents(){
  const labels={online:'在线',stale:'连接延迟',offline:'失联',revoked:'已撤销'};
  $('#agent-grid').innerHTML=state.agents.map(a=>{const change=a.domainChange,working=change&&['pending','applying'].includes(change.state),failed=change?.state==='failed',local=a.transport==='loopback',isIp=a.proxyMode==='ip',inputDomain=working||failed?(change.mode==='ip'?'':change.desiredDomain):(isIp?'':a.domain),targetLabel=change?(change.mode==='ip'?'IP 反代':change.desiredDomain||''):'',status=change?`<div class="node-test show"><span class="${failed?'bad':working?'warn':'good'}">●</span>&nbsp;${escapeHtml(change.message||'')}${targetLabel&&targetLabel!==a.domain?` · 目标 ${escapeHtml(targetLabel)}`:''}</div>`:'';const probeVer=local?state.panelVersion:(a.agentVersion||''),probeCurrent=!!state.panelVersion&&probeVer===state.panelVersion;return`<article class="node-card agent-card" data-id="${escapeHtml(a.id)}" data-status="${escapeHtml(a.status)}"><div class="node-card-head"><span class="drag-handle" title="拖动调整机器顺序" draggable="true"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><circle cx="9" cy="6" r="1"/><circle cx="9" cy="12" r="1"/><circle cx="9" cy="18" r="1"/><circle cx="15" cy="6" r="1"/><circle cx="15" cy="12" r="1"/><circle cx="15" cy="18" r="1"/></svg></span><div style="min-width:0;flex:1"><h3><input class="agent-name" value="${escapeHtml(a.name||'')}" maxlength="80" title="直接修改名称，保存机器配置后生效"></h3><span class="node-path">${escapeHtml(a.transport==='loopback'?'本机环回':'安全轮询')}</span></div><span class="pill ${a.inSync?'on':'warn'}">${a.inSync?'配置已同步':'等待同步'}</span>${a.agentVersion&&state.panelVersion&&a.agentVersion!==state.panelVersion?`<span class="pill warn" title="该机器仍在运行旧版本代理程序">待升级 ${escapeHtml(a.agentVersion)}</span>`:''}</div><div class="tag-row"><span class="pill ${a.status==='online'?'on':'warn'}">${labels[a.status]||'失联'}</span>${local?'':`<span class="pill ${isIp?'':'info'}">${isIp?'IP 反代':'HTTPS 域名'}</span>`}<span class="pill ${working?'warn':a.applyState==='active'?'info':'warn'}">${working?(change.mode==='ip'?'正在切换 IP 反代':'正在配置域名'):a.applyState==='active'?'代理运行中':'已注册 · 等待配置'}</span></div><div class="node-meta"><div><small>当前生效域名</small><strong>${isIp?('IP 反代'+(a.domain?' · '+escapeHtml(a.domain):'')):escapeHtml(a.domain||'尚未设置')}</strong></div><div><small>已选节点</small><strong>${a.routeIds.length} 个</strong></div><div><small>探针·反代版本</small><strong>${probeVer?`v${escapeHtml(probeVer)}${state.panelVersion?` <span style="color:var(--${probeCurrent?'green':'amber'})">· ${probeCurrent?'最新':'待升级'}</span>`:''}`:'未知'}</strong></div><div><small>最近回执</small><strong>${a.lastSeen?new Date(a.lastSeen).toLocaleString():'尚未连接'}</strong></div></div><div class="agent-editor"><label>代理域名<input class="agent-domain" value="${escapeHtml(inputDomain||'')}" placeholder="${local?'emby.example.com':'留空＝用本机 IP 反代'}" ${working?'disabled':''}><small>${local?'保存后自动配置 Nginx、申请证书并停用旧代理域名，必须与面板域名不同。':'填写域名自动申请证书走 HTTPS；留空则以本机公网 IP 明文 HTTP 反代。'}</small></label><details class="agent-routes"><summary>部署节点<span class="routes-count">已选 ${a.routeIds.length} / ${state.routes.length}</span></summary>${state.routes.length?`<div class="route-tools"><button type="button" class="route-bulk route-all">全选</button><button type="button" class="route-bulk route-none">全不选</button>${state.routes.length>8?'<input class="agent-route-search" placeholder="搜索节点">':''}</div>`:''}<div class="agent-route-list">${state.routes.map(r=>`<label class="${a.routeIds.includes(r.id)?'checked':''}" data-search="${escapeHtml((r.name+' '+r.alias).toLowerCase())}"><input type="checkbox" data-route-id="${r.id}" ${a.routeIds.includes(r.id)?'checked':''}><span><b>${escapeHtml(r.name)}</b><small>/${escapeHtml(r.alias)}/</small></span></label>`).join('')||'<small>请先创建 Emby 节点</small>'}</div></details><div class="node-actions"><button class="btn primary save-agent" ${working?'disabled':''}>${a.automaticDomainSwitch?'保存并切换域名':'保存机器配置'}</button>${a.canDelete?'<button class="btn ghost danger delete-agent">从面板删除</button>':''}</div></div>${status||a.error?status||`<div class="node-test show"><span class="bad">●</span>&nbsp;${escapeHtml(a.error)}</div>`:''}${a.agentVersion&&state.panelVersion&&a.agentVersion!==state.panelVersion?`<div class="node-test show"><span class="warn">●</span>&nbsp;代理程序 ${escapeHtml(a.agentVersion)} 落后于面板 ${escapeHtml(state.panelVersion)}，在该机器执行：<code>sudo aegis-relay-agent update</code></div>`:''}</article>`}).join('')||'<article class="panel empty-state"><svg viewBox="0 0 24 24"><circle cx="12" cy="5" r="2"/><circle cx="5" cy="18" r="2"/><circle cx="19" cy="18" r="2"/><path d="M12 7v4M6.7 16.8l3.8-4M17.3 16.8l-3.8-4"/></svg><h3>添加第一台代理机器</h3><p>生成一次性安装指令，机器注册后会自动出现。</p><button class="btn primary open-agent">添加机器</button></article>';
  $('#agent-grid').querySelectorAll('.agent-card').forEach(card=>{const agent=state.agents.find(item=>item.id===card.dataset.id);const search=card.querySelector('.agent-route-search');if(search)search.oninput=()=>{const q=search.value.trim().toLowerCase();card.querySelectorAll('.agent-route-list label').forEach(l=>{l.style.display=!q||(l.dataset.search||'').includes(q)?'':'none'})};const counter=card.querySelector('.routes-count'),syncRoutes=()=>{card.querySelectorAll('[data-route-id]').forEach(b=>b.closest('label').classList.toggle('checked',b.checked));if(counter)counter.textContent=`已选 ${card.querySelectorAll('[data-route-id]:checked').length} / ${state.routes.length}`};card.querySelectorAll('[data-route-id]').forEach(box=>box.addEventListener('change',syncRoutes));const allBtn=card.querySelector('.route-all'),noneBtn=card.querySelector('.route-none');if(allBtn)allBtn.onclick=()=>{card.querySelectorAll('[data-route-id]').forEach(b=>b.checked=true);syncRoutes()};if(noneBtn)noneBtn.onclick=()=>{card.querySelectorAll('[data-route-id]').forEach(b=>b.checked=false);syncRoutes()};card.querySelector('.save-agent').onclick=async()=>{try{const routeIds=[...card.querySelectorAll('[data-route-id]:checked')].map(input=>input.dataset.routeId),domain=card.querySelector('.agent-domain').value.trim(),name=card.querySelector('.agent-name').value.trim(),currentInput=agent.proxyMode==='ip'?'':(agent.domain||''),modeChanged=domain!==currentInput;if(!name)return toast('机器名称不能为空',true);await call(`/agents/${agent.id}`,{method:'PATCH',body:JSON.stringify({name,domain,routeIds})});toast(!modeChanged?'代理机器配置已保存':(domain?'域名切换已提交，正在申请证书':'已提交：切换为 IP 反代（HTTP）'));await refreshAll();await loadAgents();if(modeChanged)watchDomainSwitch()}catch(e){toast(e.message,true)}};bindAgentDrag(card,agent);const remove=card.querySelector('.delete-agent');if(remove)remove.onclick=async()=>{if(!confirm(`确认从面板删除“${agent.name}”？请先在对应机器执行卸载。`))return;try{await call(`/agents/${agent.id}`,{method:'DELETE'});toast('代理机器已从面板删除');await loadAgents()}catch(e){toast(e.message,true)}}});
...
async function switchPage(page){state.page=page;$$('.page').forEach(x=>x.classList.add('hidden'));$(`#${page}-page`).classList.remove('hidden');$$('[data-page]').forEach(x=>x.classList.toggle('active',x.dataset.page===page));$('#page-title').textContent=titles[page];$('#breadcrumb').textContent=`控制台 / ${titles[page]}`;const canAdd=page==='dashboard'||page==='nodes'||page==='agents';$('#global-add').style.display=canAdd?'':'none';$('#global-add').innerHTML=page==='agents'?'<svg viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>添加机器':'<svg viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>新增节点';$('#global-add').onclick=page==='agents'?openAgentModal:()=>openDrawer();closeMenu();if(page==='audit')await loadAudit();if(page==='agents')await loadAgents();if(page==='deployment')await loadDeployment();if(page==='notifications')await loadNotifications();if(page==='account')await loadAccount();if(page==='diagnostics')await loadRuntime();if(page==='access')await loadAccess()}
async function loadAccess(){const host=$('#access-list');host.innerHTML='<article class="panel empty-state"><h3>正在汇总访问记录…</h3></article>';try{const d=await call('/access');const nodes=(d.nodes||[]).filter(n=>state.routes.some(r=>r.id===n.id));if(!nodes.length){host.innerHTML='<article class="panel empty-state"><h3>暂无访问记录</h3><p>有人通过反代观看后，这里会按“节点 › 分发通道 › 设备”折叠显示。</p></article>';return}
  const fmt=t=>t?new Date(Number(t)).toLocaleString():'—';
  host.innerHTML=nodes.map(n=>{
    const channels=(n.channels||[]).map(c=>{
      const rows=(c.viewers||[]).map(v=>`<tr><td><code>${escapeHtml(v.ip)}</code></td><td>${escapeHtml(v.deviceName||v.client||'—')}${v.client&&v.deviceName?`<br><small class="muted">${escapeHtml(v.client)}</small>`:''}</td><td><small class="muted">${escapeHtml((v.ua||'').slice(0,50))}</small></td><td>${escapeHtml(v.machine||'—')}</td><td>${v.hits||0}</td><td>${fmt(v.lastSeen)}</td></tr>`).join('');
      return `<details class="chan-detail ${c.suspicious?'suspicious':''}"><summary><span class="chan-name">${escapeHtml(c.label)}</span><span class="pill ${c.suspicious?'warn':'info'}">设备 ${c.distinctDevices}</span><span class="pill off" title="动态 IP 仅供参考">IP ${c.distinctIps}</span>${c.suspicious?'<span class="pill danger">⚠ 疑似转分发</span>':''}</summary>${rows?`<div class="table-wrap"><table><thead><tr><th>IP</th><th>设备 / 客户端</th><th>UA</th><th>机器</th><th>请求</th><th>最近访问</th></tr></thead><tbody>${rows}</tbody></table></div>`:'<p class="muted">该通道暂无观看记录。</p>'}</details>`;
    }).join('');
    return `<details class="panel access-node ${n.suspicious?'suspicious':''}"><summary class="access-node-head"><span class="access-node-name">${escapeHtml(n.name)} <small class="muted">/${escapeHtml(n.alias)}/</small></span><span class="node-status ${n.suspicious?'bad':'ok'}">${n.suspicious?'⚠ 疑似多人共享':'✓ 安全'}</span><span class="pill off">通道 ${(n.channels||[]).length}</span><span class="pill off">设备合计 ${n.distinctDevices}</span></summary><div class="access-node-body">${channels||'<p class="muted">暂无访问记录。</p>'}</div></details>`;
  }).join('')}catch(e){host.innerHTML=`<article class="panel empty-state"><h3>加载失败</h3><p>${escapeHtml(e.message)}</p></article>`}}
async function renderChannels(route){const section=$('#channels-section'),list=$('#channels-list'),hint=$('#channels-hint'),lbl=$('#channel-label'),add=$('#add-channel');section.style.display='';
  if(!route){list.innerHTML='';hint.textContent='保存节点后，可在此为不同用户添加分发通道。';lbl.disabled=true;add.disabled=true;return;}
  if(route.accessMode!=='key'){list.innerHTML='';hint.textContent='该节点是公开别名模式（无连接密码），无法区分多用户。请在上方“连接密码保护”开启后再来。';lbl.disabled=true;add.disabled=true;return;}
  lbl.disabled=false;add.disabled=false;hint.textContent='';
  try{const d=await call(`/routes/${route.id}/channels`),bu=String(d.publicBaseUrl||location.origin).replace(/\/$/,'');
    list.innerHTML=(d.channels||[]).map(c=>`<div class="channel-row" data-cid="${escapeHtml(c.id)}"><div class="channel-info"><strong>${escapeHtml(c.label||'默认')}</strong>${c.isDefault?' <span class="pill off">默认 / 自己</span>':''}<br><code class="muted chan-url">${escapeHtml(bu+c.clientPath)}</code></div><div class="channel-actions"><button type="button" class="btn ghost copy-chan" data-url="${escapeHtml(bu+c.clientPath)}">复制链接</button>${c.isDefault?'':'<button type="button" class="btn ghost danger del-chan">删除</button>'}</div></div>`).join('');
    list.querySelectorAll('.copy-chan').forEach(b=>b.onclick=async()=>{try{await navigator.clipboard.writeText(b.dataset.url);toast('专属链接已复制')}catch(e){toast(e.message,true)}});
    list.querySelectorAll('.del-chan').forEach(b=>b.onclick=async()=>{const cid=b.closest('.channel-row').dataset.cid;if(!confirm('删除该通道后，用这把密码的用户会立即无法访问（不影响其他人）。确认删除？'))return;try{await call(`/routes/${route.id}/channels/${cid}`,{method:'DELETE'});await renderChannels(route);toast('通道已删除')}catch(e){toast(e.message,true)}});
  }catch(e){list.innerHTML='';hint.textContent=e.message}}
