...
async function persistAgentOrder(){const updates=state.agents.map((agent,i)=>({agent,order:(i+1)*10})).filter(x=>Number(x.agent.sortOrder)!==x.order);for(const {agent,order} of updates){await call(`/agents/${agent.id}`,{method:'PATCH',body:JSON.stringify({sortOrder:order})});agent.sortOrder=order}}
