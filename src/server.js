...
function agentView(agent){const view=publicAgent(agent,store.data,agent.id===LOCAL_AGENT_ID?localAgent.status():null);return agent.id===LOCAL_AGENT_ID?{...view,domain:configuredLocalDomain||view.domain||'',domainChange:localDomainStatus(),automaticDomainSwitch:true}:view}
...
async function api(req, res, rel, cookiePath = cfg.adminPath) {
  ...
  if (req.method === 'POST' && rel === '/routes') {
    ...
    store.data.routes.push(r);ensureLocalDeployment(store.data,r.id,now);store.audit('route.created',ip(req),alias);localAgent.reconcile();return json(res,201,{route:publicRoute(r),...credentialsFor(r,accessKey)});
  }
