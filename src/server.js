...
const ui = fs.readFileSync(path.join(ROOT, 'web', 'index.html'),'utf8').replace('APP_JS',`${cfg.adminPath}/app.js`).replace('STYLE_CSS',`${cfg.adminPath}/style.css`);
...
async function api(req, res, rel) {
  ...
  if (req.method === 'GET' && rel === '/events') {res.writeHead(200,headers({'content-type':'text/event-stream','connection':'keep-alive'}));const send=()=>res.write(`event: dashboard\ndata: ${JSON.stringify(metrics.snapshot(store.data.routes))}\n\n`);send();const off=metrics.subscribe(send),timer=setInterval(()=>res.write(': heartbeat\n\n'),15000);req.on('close',()=>{off();clearInterval(timer)});return;}
