...
async function api(req, res, rel, cookiePath = cfg.adminPath) {
  ...
  if (req.method === 'POST' && rel === '/routes') {
    ...
    const r={id:randomToken(12),alias,name:String(b.name||alias).slice(0,80),upstreams,playbackUpstreams,allowPrivate,tlsVerify:b.tlsVerify!==false,enabled:true,accessMode,showOnHome:b.showOnHome===true,clientProfile:cleanProfile(b.clientProfile),streamRewrite:cleanStreamRewrite(b.streamRewrite),tags:cleanTags(b.tags),notes:String(b.notes||'').slice(0,500),favorite:b.favorite===true,sortOrder:numeric(b.sortOrder,-10000,10000),speedLimitMbps:numeric(b.speedLimitMbps,0,100000),monthlyQuotaGB:numeric(b.monthlyQuotaGB,0,1000000),reminderDays:numeric(b.reminderDays,0,365),...routeAuthFields(accessKey),createdAt:now,updatedAt:now};
