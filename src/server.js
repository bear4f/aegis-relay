...
async function api(req, res, rel, cookiePath = cfg.adminPath) {
  ...
  if (req.method === 'GET' && rel === '/agents') return json(res,200,{agents:[localAgent.status()]});
  ...
  if (req.method === 'POST' && rel === '/routes') {
    ...
    store.data.routes.push(r);store.audit('route.created',ip(req),alias);localAgent.reconcile();return json(res,201,{route:publicRoute(r),...clientCredentials(r,accessKey)});
  }
