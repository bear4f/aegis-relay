...
function dashboardSnapshot(){
  const localSnapshot=metrics.snapshot(localAgent.routeSource.getRoutes()),localTelemetry=telemetryFromMetrics(localSnapshot),localRecord=store.data.agents.find(agent=>agent.id===LOCAL_AGENT_ID)||{id:LOCAL_AGENT_ID,name:'主代理机器'},localView=publicAgent(localRecord,store.data,localAgent.status());
  let localDomain='';try{localDomain=new URL(cfg.publicBaseUrl).host}catch{}
...
async function api(req, res, rel, cookiePath = cfg.adminPath) {
  ...
  if (req.method === 'POST' && rel === '/routes') {
    ...
    store.data.routes.push(r);ensureLocalDeployment(store.data,r.id,now);store.audit('route.created',ip(req),alias);localAgent.reconcile();return json(res,201,{route:publicRoute(r),...clientCredentials(r,accessKey)});
  }
  const agentMatch=rel.match(/^\/agents\/([^/]+)$/);if(agentMatch){const agent=store.data.agents.find(item=>item.id===agentMatch[1]);if(!agent)return json(res,404,{error:'agent not found'});
    if(req.method==='PATCH'){const b=await body(req),now=new Date().toISOString();if(b.name!==undefined){const name=String(b.name||'').trim();if(!name||name.length>80)return json(res,400,{error:'invalid agent name'});agent.name=name;}if(b.domain!==undefined)agent.domain=normalizeAgentDomain(b.domain);if(b.routeIds!==undefined){if(!Array.isArray(b.routeIds))return json(res,400,{error:'routeIds must be an array'});const routeIds=[...new Set(b.routeIds.map(String))];if(routeIds.some(id=>!store.data.routes.some(route=>route.id===id)))return json(res,400,{error:'unknown route id'});replaceAgentDeployments(store.data,agent.id,routeIds,now);}agent.updatedAt=now;if(agent.id===LOCAL_AGENT_ID)localAgent.reconcile();store.audit('agent.updated',ip(req),agent.id);return json(res,200,{agent:publicAgent(agent,store.data,agent.id===LOCAL_AGENT_ID?localAgent.status():null)});}
    if(req.method==='DELETE'){if(agent.id===LOCAL_AGENT_ID)return json(res,409,{error:'the local agent cannot be deleted'});store.data.agents=store.data.agents.filter(item=>item!==agent);store.data.deployments=store.data.deployments.filter(item=>item.agentId!==agent.id);store.audit('agent.deleted',ip(req),agent.id);return json(res,200,{ok:true});}
  }
  const m=rel.match(/^\/routes\/([^/]+)$/); if(m){const r=store.data.routes.find(x=>x.id===m[1]);if(!r)return json(res,404,{error:'not found'});
    ...
    if(req.method==='DELETE'){store.data.routes=store.data.routes.filter(x=>x!==r);removeRouteDeployments(store.data,r.id);store.audit('route.deleted',ip(req),r.alias);localAgent.reconcile();return json(res,200,{ok:true});}
  }
