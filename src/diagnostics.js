...
export async function diagnoseRoute(route){const main=route.upstreams?.length?route.upstreams:(route.upstream?[route.upstream]:[]);const options={allowPrivate:route.allowPrivate,tlsVerify:route.tlsVerify!==false};const streamTargets=streamProbeTargets(route);return{node:{id:route.id,alias:route.alias,name:route.name},main:await Promise.all(main.map(x=>probe(x,options))),stream:streamTargets.length?await Promise.all(streamTargets.map(x=>probe(x,options))):null,headerPreview:route.clientProfile?.enabled?{userAgent:route.clientProfile.userAgent||'',client:route.clientProfile.client||'',deviceName:route.clientProfile.deviceName||'',deviceId:route.clientProfile.deviceId?'[configured]':''}:{mode:'pass-through'}}}
