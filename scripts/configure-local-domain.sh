...
restore_site() {
  if [ -n "$BACKUP" ] && [ -f "$BACKUP" ]; then cp "$BACKUP" "$SITE"; nginx -t >/dev/null 2>&1 && systemctl reload nginx || true; fi
}
...
if ! certbot --nginx -d "$PANEL_DOMAIN" --non-interactive --agree-tos --redirect -m "$EMAIL"; then
  restore_site; echo "面板域名证书配置失败，已恢复旧配置。" >&2; exit 1
fi
if ! certbot --nginx -d "$PROXY_DOMAIN" --non-interactive --agree-tos --redirect -m "$EMAIL"; then
  restore_site; echo "代理域名证书申请失败，已恢复旧配置。请检查 DNS、Cloudflare 和 80/443 端口。" >&2; exit 1
fi
