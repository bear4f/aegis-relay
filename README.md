...
- **v0.8.8 播放快路径**：所有播放请求默认按原始字节零改动转发，即使上游把 `200` 视频错误标成文本或 JSON 也不会进入正文改写；只有明确的 HLS/DASH 清单例外。短 API 请求继续复用连接；媒体正文、Range/拖动与跳转后的实际取流使用独立上游连接，避免部分 Emby 前端/CDN 在复用 API 连接后只返回几 KiB 就停顿。主代理与探针共用同一逻辑。
