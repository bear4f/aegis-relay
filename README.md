...
- **v0.10.9 探针注册以令牌为准 + 真实健康上报**：注册幂等由**令牌**决定，而不是"identity 文件是否存在"——带一枚**有效新令牌**重跑安装会重新注册本机（覆盖旧身份，适配换机/重装），令牌**已用/过期**且本机已注册则保留现有身份、不再抛「agent is already enrolled」堆栈；重新注册时清理旧密钥封装的缓存快照，让探针从面板重新同步（修复 v0.10.8「有旧 identity 就跳过注册，导致新机器不显示、502」的回归）。身份文件本就在控制面确认注册之后才原子落盘，不存在"文件在、控制面没有"的半注册态。探针上报的 `proxyHealthy` 改为真实值（代理监听器出错/关闭即置假并记日志，不再永远显示健康），优雅退出增加 `closeIdleConnections` + 强制超时，避免长连接把 `docker compose down` 拖到 10 秒强杀。
