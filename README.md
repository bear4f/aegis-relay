...
- **v0.10.2 播放兼容快路径**：所有播放响应默认按原始字节零改动转发；短 API/`PlaybackInfo` 继续复用连接，而首播、HLS 分片和每次 Range 拖动都使用独立干净的上游连接，避免部分 Emby/CDN 在复用控制或上一段媒体连接后只传几十 KB 就停顿。HTTPS 媒体仍复用 TLS 会话以缩短握手；明确长度的媒体中途断流时按已送达字节校验续拉，静默恢复窗口由 30 秒缩短为 8 秒。主代理与探针共用同一逻辑。
