...
- **v0.10.9 探针注册以令牌为准 + 真实健康上报**：注册幂等由**令牌**决定，而不是"identity 文件是否存在"——带一枚**有效新令牌**重跑安装会重新注册本机（覆盖旧身份，适配换机/重装），令牌**已用/过期**且本机已注册则保留现有身份、不再抛「agent is already enrolled」堆栈；重新注册时清理旧密钥封装的缓存快照，让探针从面板重新同步（修复 v0.10.8「有旧 identity 就跳过注册，导致新机器不显示、502」的回归）。**v0.10.10** 进一步在重装/重新注册时清理上一辈子残留的 `host-domain-status`/`request`，否则探针会一直上报旧域名（换机复用同一台 VPS 时面板显示的还是上一次的域名）。身份文件本就在控制面确认注册之后才原子落盘，不存在"文件在、控制面没有"的半注册态。探针上报的 `proxyHealthy` 改为真实值（代理监听器出错/关闭即置假并记日志，不再永远显示健康），优雅退出增加 `closeIdleConnections` + 强制超时，避免长连接把 `docker compose down` 拖到 10 秒强杀。
- **v0.10.12 新建节点可选部署机器**：新建节点抽屉里可直接选「部署到代理机器」，创建即部署到该机器,弹出的复制地址立刻用**该机器的反代域名**而不是面板域名（此前新节点尚未分配到任何远端机器,只能回落面板域名）；仅一台远端机器时默认选中它。之后仍可在「代理机器」里调整分配。
- **v0.10.13 探针容器重建自愈**：`--force-recreate` 会先停旧容器，若新容器因「port is already allocated」起不来，机器就会**完全没有探针在跑**、在面板显示失联。现在升级/安装/切域名/切 IP 的每条重建路径都会自动兜底：清理项目、强制移除残留容器（常见于 Compose v1 `项目_服务_1` 与 v2 `项目-服务-1` 两套命名互不识别）后重试；`./data` 是宿主机挂载，注册身份与缓存配置不受影响。
- **v0.10.14 端口自愈补齐孤儿 docker-proxy**：容器非正常退出时 Docker 会遗留负责端口映射的 `docker-proxy` 进程，它在**没有任何容器**的情况下继续占着 8080——只删容器救不回来。现在重建失败会依次尝试：清理残留容器 → **清理占用本机发布地址 8080 的孤儿 docker-proxy** → 再启动，仍失败才报错并提示 `systemctl restart docker`。只会清理与本探针发布地址完全匹配的映射，不碰其他容器的端口。
...
- 机器名称**点标题就能改**；卡片可**拖动排序**（与节点管理一致，顺序保存在面板，不影响探针）
