...
- **v0.8.7 播放快路径**：所有播放请求默认按原始字节零改动转发，即使上游把 `200` 视频错误标成文本或 JSON 也不会进入正文改写；只有明确的 HLS/DASH 清单例外。主代理与探针共用同一快路径，并保留 Range/206、长连接、连接池、TCP NoDelay 和快速故障线路切换。
- 面向流媒体调优：上下游连接复用、DNS 结果缓存、256 KiB 有界背压窗口、Nginx 256 KiB 无缓冲读取块、无限速时零中间变换、`TCP_NODELAY`、长空闲窗口（不会掐断慢转码和暂停）
...
> **更新策略时，主机和每台代理机都要各自更新**——数据面代码是共用的，但每台机器各自构建镜像。面板会把落后的机器标为「待升级」，照着提示逐台执行即可。v0.8.7 起两条更新命令都会重建当前 Nginx 入口，使连接复用和大块流式转发调优同时生效。
