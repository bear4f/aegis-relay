...
> 当前为安全预览版 `0.6.1`。首次部署建议先在测试域名验证，再承载正式节点。
...
`0.6.1` 已完成远程 Agent 的安全注册、签名轮询、按机器加密快照、离线缓存、原子应用、ACK 回执和实际播放数据面。统一证书邮箱只需在“部署向导”保存一次；之后每台代理机生成的一键指令会自动完成注册、节点同步、Nginx、Let's Encrypt 证书和自动续期。控制面离线时，Agent 继续使用最后一份已验签配置；现有本机反代不受影响。
